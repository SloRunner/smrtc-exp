...
//DB index = search_index {"index":1.0}
...
module.exports = {
  ...
  update_masternode: function (params, cb) {
    ...
    Masternode.update({ txhash: params.txhash, outidx: params.outidx }, {
      rank: params.rank,
  ...
  {
    Masternode.remove({lastseen:{$lte:(Math.floor(Date.now() / 1000) - 86400)}}, function (err) {
      if (err) {
        console.log(err);
        return cb();
      } else {
